...
const UsersPage = () => {
    ...
    if (session?.user.token.token) {
      if (session?.user.user.role === 'admin') {
        const fetchUsers = async () => {
          try {
            const usersData = await UserService.getAllUsers(session.user.token.token);
            setUsers(usersData);
  ...
  const handleDelete = async (userId: number) => {
    try {
      ...
      if (session?.user.token?.token) {
        await UserService.deleteUser(userId, session.user.token.token);
  ...
  const handleUpdateUser = async (updatedUser: Partial<User>) => {
    try {
      if (session?.user.token?.token && selectedUser) {
        const updated = await UserService.updateUser(selectedUser.id, updatedUser, session.user.token.token);
  ...
  const handleCreateUser = async (newUser: Partial<User>) => {
    try {
      if (session?.user.token?.token) {
        const createdUser = await UserService.createUser(newUser, session.user.token.token);
